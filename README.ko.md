...
- **토큰 단위 스트리밍 + 작업 중 추가 지시**: 답변이 실시간으로 흘러나옵니다. 실행 중에도 지시를 추가하거나 추가 질문을 콜드 스타트 없이 보낼 수 있고, 다른 세션은 병렬 실행
- 키 기반 로그인(최초 실행 시 자동 생성)
- 깔끔한 Markdown 렌더링(DOMPurify 정화), 코드 하이라이트, **이미지 붙여넣기／선택해 Claude에게 보여주기**(비전)
- Claude가 생성한 이미지/오디오/영상/PDF 보기, dev server를 휴대폰에서 미리보기; **정적 HTML 프로토타입을 휴대폰에서 바로 렌더링**(샌드박스화, 직접 서버를 띄울 필요 없음) — 미리보기는 모두 앱 안에서 「뒤로」 버튼과 함께 열려, 화면을 덮어 돌아오지 못하는 일이 없습니다
- **도구 패널**: 사용량/비용 통계 · 생성물 미디어 갤러리 · 여러 서버 원탭 전환
- 세션을 **고정 / 보관** 가능 — 목록이 길어도 찾기 쉬움
...
- 사이드바 **파일** 버튼으로 세션 폴더 탐색, `.md`는 내장 리더로 열림, `.html`은 휴대폰에서 **웹페이지로 렌더링**(샌드박스화).
- 우측 상단 **⊞ 도구**: **사용량**(프로젝트별 비용 · PocketClaude로 보낸 대화만 집계), **갤러리**(모든 세션에서 생성한 이미지/오디오/영상의 벽), **서버**(여러 PocketClaude를 저장하고 키를 지닌 채 원탭 전환).
- 세션 행을 **고정**(맨 위로) 또는 **보관**(맨 아래로 + 흐리게)할 수 있으며, 이 설정은 브라우저에 로컬 저장됩니다.

### 참고 / 알려진 제한

- **지속 스트리밍 세션**: 답변이 토큰 단위로 표시됩니다. 실행 중 세션에 보내면 라이브 프로세스로 전달되어 현재 턴 이후 실행됩니다(콜드 스타트 없음). 다른 세션은 병렬, 각 프로세스는 5분 유휴 후 종료.
...
- **업데이트 후 원격 기기가 옛 버전을 표시하면**: 한 번 새로고침하면 해결됩니다(PWA service worker가 버전을 교체) — 드물게 두 번 필요합니다. 버전 표시는 ⚙ 고급 옵션 맨 아래(vX.Y.Z)에 있어, 기기가 어떤 빌드를 로드했는지 확인할 수 있습니다.
- **광고 차단기**: UI 요소 이름은 ad/adv 등을 의도적으로 피합니다(광고 차단기의 코스메틱 필터가 「광고」로 오인해 숨기는 것을 방지). UI를 커스터마이즈한다면 이런 이름을 쓰지 마세요.
...
- 모든 Markdown 출력은 DOMPurify 정화. `/media`는 텍스트 파일(HTML 포함)을 `text/plain`으로 제공. `.html`을 실제 페이지로 미리보려면 `/html` 사용 — **`sandbox` CSP**(opaque origin)로 렌더링: 자체 JS는 실행되지만 인증 쿠키에 접근할 수 없고 동일 출처 API에도 도달할 수 없음.
