...
- **更新後にリモート端末が古い版を表示する場合**：一度リロードすれば直ります（PWA の service worker がバージョンを切り替えます）。稀に 2 回必要なことがあります。バージョン印は ⚙ 詳細オプションの最下部（vX.Y.Z）にあり、端末がどのビルドを読み込んだか確認できます。
- **広告ブロッカー**：UI 要素の名前は `ad` / `adv` 等を意図的に避けています（広告ブロッカーの美化フィルタが「広告」と誤判定して隠すのを防ぐため）。UI をカスタマイズする場合はこうした名前を使わないでください。
