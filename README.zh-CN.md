...
- **逐字流式 + 中途插话**：Claude 的回复逐字实时显示；任务跑到一半可补指示、追问免冷启动；不同会话并行
- 密钥登录保护（首次启动自动生成）
- 漂亮的 Markdown 渲染（DOMPurify 消毒）、代码高亮、**贴图／选图传给 Claude 看**（视觉识别）
- 查看 Claude 产出的图片/音乐/视频/PDF、一键在手机预览 dev server；**静态 HTML 原型直接在手机渲染**（沙箱化，免自己起服务器）——预览都在 App 内以「返回」键浏览，不会被盖掉切不回来
- **工具面板**：用量花费统计 · 产出媒体画廊 · 多台服务器一键切换
- 会话可**置顶 / 归档**，清单再长也好找
...
- 侧栏的**文件**按钮可浏览会话目录，`.md` 用内置阅读视图打开，`.html` 会在手机上**渲染成网页**（沙箱化）。
- 右上角 **⊞ 工具**：**用量**（每个项目花了多少 · 只统计透过 PocketClaude 送出的对话）、**画廊**（所有 session 产出的图/音/影一整面墙）、**服务器**（存多台 PocketClaude，一键带密钥切换）。
- 会话列表上可**置顶**或**归档**（沉底淡化），这些设置存在浏览器本机。

### 注意事项 / 已知限制

- **持久流式会话**：回复逐字实时显示；对进行中的会话发消息会实时串入（排到下一回合执行），追问无需冷启动。不同会话并行；每个进程闲置 5 分钟后自动退出。
...
- 更新后远端看到旧版：重载一次即可（PWA 的 service worker 换版），偶尔要两次。版本印记在 ⚙ 进阶选项最下方（vX.Y.Z），可用来确认设备载到哪一版。
- 广告拦截器：界面元素刻意避开 ad/adv 等命名，以免被拦广告误判隐藏；若你自订 UI，别用这类名称。
...
- 所有 Markdown 输出经 DOMPurify 消毒；`/media` 的文本文件（含 HTML）一律以 `text/plain` 提供。要把 `.html` 当网页预览时走 `/html`——以 **`sandbox` CSP**（opaque origin）渲染：跑得了自己的 JS，但碰不到登录 cookie，也打不进同源 API。
